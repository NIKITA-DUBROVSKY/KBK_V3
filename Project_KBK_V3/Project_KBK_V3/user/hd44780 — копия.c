...
{
    uint16_t us_count_tic =  us * TICKs_us; // умножаем на наше значение на тики
    TIM16->CNT = 0U; // обнуляем счётчик
    while(TIM16->CNT < us_count_tic);
}
...
//	
//	lcd44780_GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
//	lcd44780_GPIO_InitStructure.GPIO_Pin = 	lcd44780_pin_E | lcd44780_pin_D4 | lcd44780_pin_D5 | lcd44780_pin_D7 | lcd44780_pin_D6;
//	lcd44780_GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
//	lcd44780_GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
//	lcd44780_GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;	
//	GPIO_Init(GPIOA, &lcd44780_GPIO_InitStructure);
//	
//	lcd44780_GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
//	lcd44780_GPIO_InitStructure.GPIO_Pin = 	lcd44780_pin_RS;
//	lcd44780_GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
//	lcd44780_GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
//	lcd44780_GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;	
//	GPIO_Init(GPIOB, &lcd44780_GPIO_InitStructure);
//}
//************************************************************************//
...
//	lcd44780_E_1;
//	GPIOA->BSRR = (data) | (data_inv << 16); //выставляем данные на ножки контроллера (0 пин соответствует ножке данных D4, 1 пин - D5 и т.д.)
...
//	GPIOA->BSRR = (data << 9) | (data_inv << 25); //выставляем данные на ножки контроллера (9 пин соответствует ножке данных D4, 10 пин - D5 и т.д.)
//	delay_micros(10); //задержка для строба
	lcd44780_EN_0;
	delay_micros(50); //задержка для контроллера дисплея, чтобы тот успел записать данные в свою RAM
}
...
void lcd44780_init(void)
{
	lcd44780_EN_0;
	lcd44780_RS_0;
	delay_micros(1600);delay_micros(1600);delay_micros(1600);delay_micros(1600);
	delay_micros(1600);delay_micros(1600);delay_micros(1600);delay_micros(1600);
	delay_micros(1600);delay_micros(1600);// потому что функция не может принят значение больше (65535/(SysClock/1000000)) 
